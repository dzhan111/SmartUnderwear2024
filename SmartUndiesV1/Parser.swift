...
func parseData(atURL: URL) {
    ...
    //since we need this if let declaration, content is not initialized outside of the scope of this func
    //but, once we leave this scope, content will revert back to its original initialization
    ...
    var time1: [String] = []
    var unfiltered_high_gain: [String] = []
    
    var time2: [String] = []
    var unfiltered_high_gain2: [String] = []
    ...
    func flashRead(addr: Int, bytesNum: Int) -> Int {
        ...
        if bytesNum == 4 {
            ...
            for i in 0..<bytesNum {
                ...
                toreconstitute[i] = Int(content[readAddr])
                //print(readAddr)
                //print(content[readAddr])
                //print(content)
                
            }//****************content has some nil value
    ...
    func printData(trackingNum: Int) {
        ...
        do {
            ...
            for i in 0..<table_data2[1].count {
                ...
                print("\(row),\(rowData2),\(rowData3),\(rowData4)")
...
//problems for 6/30
/*
 - the save function is not saving
 - the table_data1 and table_data2 are not being filled up
 - we should remove unnecessary debug statements
 - comment code
 - get sub 3.1 tta
 */
