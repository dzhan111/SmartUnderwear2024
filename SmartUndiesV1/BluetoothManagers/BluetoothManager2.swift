...
class BluetoothViewModel: NSObject, ObservableObject {
    ...
    func displayReceivedData(_ data: Data) {
        ...
        for integerValue in integers {
            ...
            //checks for 4 nulls in a row. First time, it clears file.
            //Second time, it will have recieved all data -> dc -> parse
            if(nullCount >= 7){
                if(firstRound){
                    nullCount = 0
                    clearFile()
...
extension BluetoothViewModel: CBPeripheralDelegate {
    ...
    func peripheral(_ peripheral: CBPeripheral, didUpdateValueFor characteristic: CBCharacteristic, error: Error?) {
        //send start data transmission string here
        if(count < 1){
            sendDataToDevice("D")
            print("sent D")
            count+=1
    ...
    func sendDataSignal(){
        sendDataToDevice("A")
        print("sent start data transmission : A")
